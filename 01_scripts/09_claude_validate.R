...
claude_sub <- claude_extract %>% filter(doi %in% target_dois)
hand_sub   <- hand_extract   %>% filter(doi %in% target_dois)
...
# select only the data from claude that is

# not excluded from hand
# not excluded from claude
# has an auc value
