...
term_1 <- sprintf('(%s) AND ("2024/04/01"[Date - Publication] : "2024/04/30"[Date - Publication])', search_filter)
...
# this is the total number of articles on medline in Apr 2024 with the search term 
save(article_summaries, file = "02_data/02_pilot/pilot_article.Rdata")
...
# save screening data
save(screening_data, file = "02_data/02_pilot/pilot_screening_data.Rdata")
...
for (i in 1:length(as.numeric(screening_data$uid))){
  ...
  Sys.sleep(0.2)  # Pause for 0.1 second between requests to respect rate-limiting
}
...
# and filter the epub date to only those published in 2024 Apr
pubmed_final <- rand_order %>% 
  filter(epubdate > 20240400 & epubdate < 20240431)

# filter the results to only those which have a epub date of 2024 Apr
...
write.csv(rayyan_screen, file = "02_data/02_pilot/pilot_pubmed_final.csv", row.names = FALSE)
